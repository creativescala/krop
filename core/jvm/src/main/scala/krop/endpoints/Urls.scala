...
trait Urls extends algebra.Urls with StatusCodes {
  ...
  override def optQsWithDefault[A](
      name: String,
      default: A,
    ...
    qs(name, docs)(optionalQueryStringParam(value)).xmap(_.getOrElse(default))(
      Some(_)
    )
  ...
  def segment[A](name: String = "", docs: Documentation = None)(implicit
    ...
    new Path[A] {
      ...
      ): Option[(Validated[A], List[String])] = {
        segments match {
          case head :: tail =>
            val validatedA =
              A.decode(head)
  ...
  private def pathExtractor[A](
    ...
    val segments =
      uri.path.renderString
        ...
        .toList ++ {
        if (uri.path.renderString.endsWith("/")) List("") else Nil
      }
